...
namespace Plugins.CodeMonkey.HouseBuildingSystem.Scripts
{
    public class HouseBuildingSystem : MonoBehaviour {
        ...
        [SerializeField] private CanvasHitDetector _canvasHitDetector;
        ...
        public void UpdateHouseBuildingSystemStatus(bool _isActive)
        {
            isActive = _isActive;
            DeselectObjectType();
        }
    
        public bool IsHouseBuildingSystemActive()
        {
            return isActive;
        }
        ...
        private void HandleNormalObjectPlacement() {
            if (placeObjectType == PlaceObjectType.GridObject) {
                //removed  && !UtilsClass.IsPointerOverUI() from if statement below
                if (Input.GetMouseButton(0) && placedObjectTypeSO != null && !_canvasHitDetector.IsPointerOverUI()) {
